resource "aws_lambda_function" "jobs_alert" {
  ...
  filename         = data.archive_file.jobs_alert.output_path
  source_code_hash = data.archive_file.jobs_alert.output_base64sha256
  runtime          = "ruby2.7"
...
data "archive_file" "jobs_alert" {
  type        = "zip"
  source_dir  = "app"
  output_path = "tmp/jobs_alert.zip"
}
